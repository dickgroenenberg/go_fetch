...
# This modified version of go_fetch allows usage of custom reference databases (ie. those not on genbank).
# just create a folder with your database name containing a fasta and a genbank folder
# mv custom.fasta and custom.gb (eg. exported from Geneious) to the fasta and genbank folder, respectively.
# run correct_gb.sh (this checks the formatting) on the custom.gb file
# run go_fetch_mod.py as you would when downloading from genbank.
...
if args.taxonomy != None and args.download == True:
    print(f'\n Taxonomy block passed\n')
    # print(f'\nDownloading {args.target} sequences for {args.taxonomy}:\n')
    # print(f'args.taxonomy = {args.taxonomy}')
